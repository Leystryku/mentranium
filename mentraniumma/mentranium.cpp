...
void rendershit( LPDIRECT3DDEVICE9 pDevice, const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion )
{
	...
	if ( menu && menu_initiated )
	{

			pSprite->Begin( D3DXSPRITE_ALPHABLEND );
			DWORD fvf = m_pDevice->GetFVF();
...
int D3DThread( )
{
	...
	DWORD dwD3DBase = NULL, dwResultD3DBase = NULL, *pdwVTable = NULL; 
	...
	dwResultD3DBase = dwFindPattern(dwD3DBase, (PBYTE)"\xC7\x06\x00\x00\x00\x00\x89\x86\x00\x00\x00\x00\x89\x86", const_cast<char*>("xx????xx????xx")); 

    if (dwResultD3DBase) 
    { 
        memcpy(&pdwVTable, reinterpret_cast<void*>(dwResultD3DBase + 2), 4); 
        oPresent = (tPresent)HookVTableFunction((PDWORD)(pdwVTable), (PBYTE)rendershit, 17); 
    } 
...
void __fastcall newFrameStageNotify( void *thisptr, int edx, ClientFrameStage_t curStage )
{
	...
	if ( curStage == FRAME_RENDER_START&&bIsReady )
	{
