...
public:
	int m_DrawSetColor;
	int m_DrawSetTextFont;
	int m_GetTextSize;
	int m_CreateFont;

	int m_DrawFilledRect;
	int m_DrawOutlinedRect;
	int m_DrawLine;
	int m_DrawSetTextColor;
	int m_DrawSetTextPos;
	int m_DrawPrintText;
	...
	ISurface()
	{
		...
		{
			FINDFUNC(m_DrawSetTextFont, "\x55\x8B\xEC\x8B\x45\x08\x89\x81????\x5D", 22, 0);
			FINDFUNC(m_DrawSetColor, "\x55\x8B\xEC\x8A\x45\x08\x8A\x55", 14, 0);
			FINDFUNC(m_GetTextSize, "\x55\x8B\xEC\x8B\x45\x14\x8B\x4D\x10\x8B\x55\x0C\x50\x8B\x45\x08", 78, 60);
			FINDFUNC(m_CreateFont, "\xE8????\x8B\xC8\xE9", 70, 60);

			m_DrawFilledRect	= m_DrawSetColor + 1;
			m_DrawOutlinedRect	= m_DrawSetColor + 3;
			m_DrawLine			= m_DrawSetColor + 4;

			m_DrawSetTextColor	= m_DrawSetTextFont + 2;
			m_DrawSetTextPos	= m_DrawSetTextFont + 3;
			m_DrawPrintText		= m_DrawSetTextFont + 5;
		}
	}

	DECLFUNC(m_DrawSetColor,		void, DrawSetColor,		(int r, int g, int b, int a),		(void*, int, int, int, int), (THIS, r, g, b, a))
	DECLFUNC(m_DrawFilledRect,		void, DrawFilledRect,	(int x0, int y0, int x1, int y1),	(void*, int, int, int, int), (THIS, x0, y0, x1, y1))
	DECLFUNC(m_DrawOutlinedRect,	void, DrawOutlinedRect,	(int x0, int y0, int x1, int y1),	(void*, int, int, int, int), (THIS, x0, y0, x1, y1))
	DECLFUNC(m_DrawLine,			void, DrawLine,			(int x0, int y0, int x1, int y1),	(void*, int, int, int, int), (THIS, x0, y0, x1, y1))
	
	DECLFUNC(m_DrawSetTextFont,		void, DrawSetTextFont,	(unsigned long font),				(void*, unsigned long),			(THIS, font))
	DECLFUNC(m_DrawSetTextColor,	void, DrawSetTextColor,	(int r, int g, int b, int a),		(void*, int, int, int, int),	(THIS, r, g, b, a))
	DECLFUNC(m_DrawSetTextPos,		void, DrawSetTextPos,	(int x, int y),						(void*, int, int),				(THIS, x, y))
	DECLFUNC(m_DrawPrintText,		void, DrawPrintText,	(const wchar_t* x, int l, int s),	(void*, const wchar_t*, int, int),	(THIS, x, l, s))

	DECLFUNC(m_CreateFont,		unsigned long, CreateFont,	(), (void*), (THIS))
	DECLFUNC(m_CreateFont + 1,	void, SetFontGlyphSet,		(unsigned long f, const char* n, int t, int w, int b, int s, int fl, int _min, int _max),
		(void*, unsigned long, const char*, int, int, int, int, int, int, int),
		(THIS, f, n, t, w, b, s, fl, _min, _max))

	DECLFUNC(m_GetTextSize,		void, GetTextSize,			(unsigned long f, const wchar_t* x, int& w, int& h),
...
{
public:
	ILuaShared() { Init("lua_shared", "LUASHARED", 3); }
