		...
		if(Q_strcmp(event->GetName(), "game_newmap") == 0)
		{
			...
			if ( !hookedNetchan )
			{
				...
				*(DWORD_PTR *)&channelVtbl= *(DWORD_PTR *)channel;
				...
				SendDataFn = (OriginalSendData)DetourCreate((void*)channelVtbl[41], (void*)newSendData, DETOUR_TYPE_JMP);
				hookedNetchan = true;
			}
...
int HookStuff( )
{
	...
	*(DWORD_PTR *)&inputVtbl= *(DWORD_PTR *)g_pInput;
	DetourCreate((void*)inputVtbl[8], (void*)newGetUserCmd, DETOUR_TYPE_JMP);
	...
	if ( dwC_BaseEnt_FireBullets )
		C_BaseEnt_FireBulletsFn = (OriginalC_BaseEnt_FireBullets)DetourCreate((void*)dwC_BaseEnt_FireBullets, (void*)newC_BaseEnt_FireBullets, DETOUR_TYPE_JMP);
	...
	if ( dwCNetChan_ProcessMessages )
		CNetChan_ProcessMessagesFn = (OriginalCNetChan_ProcessMessages)DetourCreate((void*)dwCNetChan_ProcessMessages, (void*)newCNetChan_ProcessMessages, DETOUR_TYPE_JMP);
...
int StartLoading( )
{
	...
	Present_O = (Present_T)DetourCreate((void*)Present, (void*)Present_H, DETOUR_TYPE_JMP);
