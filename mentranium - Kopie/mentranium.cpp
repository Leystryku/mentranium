...
DWORD NoRecoil( )
{
	return m_fLocal + m_vecPunchAngle/* + m_vecPunchAngleVel*/;
}
	...
	if ( menu->M_TriggerBot != 0 || menu->M_Aim_Correct != 0 )
	{
		...
		if ( targ )
		{
			...
			if ( menu->M_Aim_Correct )
			{
				...
				bool gothitbox = CalculateHitboxVector( targ, vec ); //mayb

				if ( gothitbox )
				{
					Vector localEyePos = GetEyePosition(pLocalEntity);
					if ( GetVisible( localEyePos, vec, targ ) ) //mayb
					{
						QAngle CorrectAng;
...
void __stdcall newSetViewAngles( QAngle &va )
{
	...
	*/
	...
	if ( menu->M_NoSpread )
	{
		...
		//if (gmod() && MENU_FIX4FAS2)
		//{
		//SetCommandNumber(cmd, );
		//return;
		//}
...
void __fastcall newShutdown( void* thisptr, int edx, const char *dc_string )
{
	...
	if ( menu->M_DisconnectSpam )
	{
		...
		const char *pussy = "";
		pussy = ballsdeep.c_str();

		return ShutdownFn( thisptr, edx, pussy );
	}
...
int CustomShouldHit( IHandleEntity* pEntityHandle, int contentsMask )
{
	...
	if ( pEntity && GetHealth(pEntity) > 0 )
	{
